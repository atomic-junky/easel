...
parser.add_option(
    ...
    "--input",
    dest="input",
    help="NSIS script to be localized",
    metavar="script.nsi",
)
parser.add_option(
    "-o",
    "--output",
    dest="output",
    help="Localized script output location",
    metavar="script.nsi",
)
parser.add_option("-p", "--podir", dest="podir", help="Directory containing PO files")
parser.add_option(
    "-l",
    "--lang",
    dest="lang",
    help="NSIS script default language (default is English)",
    default="English",
)
parser.add_option(
        ...
        if ext == ".po":
            ...
            if filename not in localeToName:
                ...
                if options.verbose:
                    print(
                        "Language: %s (%s)"
                        % (language, translationCache[language]["LANGUAGE_CODE"])
                    )
                ...
                for entry in po.untranslated_entries():
                    for label in entry.comment.split():
                        print(
                            "Warning: Label '%s' for language %s remains untranslated"
                            % (label, language)
                        )
